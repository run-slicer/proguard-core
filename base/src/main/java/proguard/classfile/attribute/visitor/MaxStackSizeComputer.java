 ...
 * visits.
 *
 * <p>This is a more memory efficient version of {@link StackSizeComputer} that doesn't store the
 * stack sizes for every offset.
 */
public class MaxStackSizeComputer
  ...
  public void visitVariableInstruction(
    ...
    exitInstructionBlock = opcode == Instruction.OP_RET;
  }
  ...
  public void visitAnySwitchInstruction(
    ...
    for (int index = 0; index < jumpOffsets.length; index++) {
      ...
      evaluateInstructionBlock(clazz, method, codeAttribute, offset + jumpOffsets[index]);
    }

    // Also evaluate the default instruction block.
    evaluateInstructionBlock(
        clazz, method, codeAttribute, offset + switchInstruction.defaultOffset);
  ...
  /**
   * A consumer of before/after stack sizes at each offset evaluated by the {@link
