...
public class DataEntryCopier implements DataEntryReader {
  ...
  /**
   * Copies all data that it can read from the given input stream to the given output stream. The
   * caller of this method will open and afterwards flush and close the input stream and the output
   * stream. The implementation of this method needs to make sure that any wrapping output streams
  ...
  public static void main(String[] args) {
    try {
      ...
      DataEntryWriter zipWriter = new ZipWriter(null, 1, false, 0, new byte[0], writer);
      if (outputIsZip) {
        // Always zip.
        writer = zipWriter;
      } else {
        ...
        writer =
            new FilteredDataEntryWriter(
                new DataEntryParentFilter(new DataEntryNameFilter(new ExtensionMatcher(".zip"))),
                zipWriter,
                writer);
      }

      // Zip up any jmods, if necessary.
      DataEntryWriter jmodWriter = new ZipWriter(null, 1, false, 0, JMOD_HEADER, writer);
      if (outputIsJmod) {
        // Always zip.
        writer = jmodWriter;
      } else {
        ...
        writer =
            new FilteredDataEntryWriter(
                new DataEntryParentFilter(new DataEntryNameFilter(new ExtensionMatcher(".jmod"))),
                jmodWriter,
                writer);
      }

      // Zip up any wars, if necessary.
      DataEntryWriter warWriter = new ZipWriter(null, 1, false, 0, new byte[0], writer);
      if (outputIsWar) {
        // Always zip.
        writer = warWriter;
      } else {
        ...
        writer =
            new FilteredDataEntryWriter(
                new DataEntryParentFilter(new DataEntryNameFilter(new ExtensionMatcher(".war"))),
                warWriter,
                writer);
      }

      // Zip up any aars, if necessary.
      DataEntryWriter aarWriter = new ZipWriter(null, 1, false, 0, new byte[0], writer);
      if (outputIsWar) {
        // Always zip.
        writer = aarWriter;
      } else {
        ...
        writer =
            new FilteredDataEntryWriter(
                new DataEntryParentFilter(new DataEntryNameFilter(new ExtensionMatcher(".aar"))),
                aarWriter,
                writer);
      }

      // Zip up any jars, if necessary.
      DataEntryWriter jarWriter = new ZipWriter(null, 1, false, 0, new byte[0], writer);
      if (outputIsJar) {
        // Always zip.
        writer = jarWriter;
      } else {
        ...
        writer =
            new FilteredDataEntryWriter(
                new DataEntryParentFilter(new DataEntryNameFilter(new ExtensionMatcher(".jar"))),
                jarWriter,
                writer);
      }

      // Zip up any apks, if necessary.
      DataEntryWriter apkWriter = new ZipWriter(null, 1, false, 0, new byte[0], writer);
      if (outputIsApk) {
        // Always zip.
        writer = apkWriter;
      } else {
        ...
        writer =
            new FilteredDataEntryWriter(
                new DataEntryParentFilter(new DataEntryNameFilter(new ExtensionMatcher(".apk"))),
                apkWriter,
                writer);
      }
      ...
      if (inputIsApk) {
        ...
        reader =
            new FilteredDataEntryReader(
                new DataEntryNameFilter(new ExtensionMatcher(".apk")), apkReader, reader);
        ...
        if (inputIsJar) {
          ...
          reader =
              new FilteredDataEntryReader(
                  new DataEntryNameFilter(new ExtensionMatcher(".jar")), jarReader, reader);
          ...
          if (inputIsAar) {
            ...
            reader =
                new FilteredDataEntryReader(
                    new DataEntryNameFilter(new ExtensionMatcher(".aar")), aarReader, reader);
            ...
            if (inputIsWar) {
              ...
              reader =
                  new FilteredDataEntryReader(
                      new DataEntryNameFilter(new ExtensionMatcher(".war")), warReader, reader);
              ...
              if (inputIsEar) {
                ...
                reader =
                    new FilteredDataEntryReader(
                        new DataEntryNameFilter(new ExtensionMatcher(".ear")), earReader, reader);
                ...
                if (inputIsJmod) {
                  ...
                  reader =
                      new FilteredDataEntryReader(
                          new DataEntryNameFilter(new ExtensionMatcher(".jmod")),
                          jmodReader,
                          reader);
                  ...
                  if (inputIsZip) {
                    ...
                    reader =
                        new FilteredDataEntryReader(
                            new DataEntryNameFilter(new ExtensionMatcher(".zip")),
                            zipReader,
                            reader);
                  }
                }
