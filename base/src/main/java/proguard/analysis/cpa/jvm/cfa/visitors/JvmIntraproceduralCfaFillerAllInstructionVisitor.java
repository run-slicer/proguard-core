 ...
 * two different tasks:
 *
 * <p>- for each exception handler (i.e. the beginning of a catch or finally block that can handle
 * several {@link ExceptionInfo}) in the attribute exception table creates a {@link
 * JvmCatchCfaNode}. Consecutive nodes may be linked with a false {@link JvmAssumeExceptionCfaEdge}
 ...
 * block of a chain an additional a false {@link JvmAssumeExceptionCfaEdge} to the exception exit
 ...
 * edge to the first catch block that handles its offset by {@link
 * JvmIntraproceduralCfaFillerVisitor}.
 *
 * <p>- sets the parameters for the current method for a {@link JvmIntraproceduralCfaFillerVisitor}
...
public class JvmIntraproceduralCfaFillerAllInstructionVisitor implements AttributeVisitor {
  ...
  public void visitCodeAttribute(Clazz clazz, Method method, CodeAttribute codeAttribute) {
    ...
    codeAttribute.instructionsAccept(
        clazz, method, new JvmIntraproceduralCfaFillerVisitor(signature, cfa));
  }

  private void generateCatchNodes(
      MethodSignature signature, CodeAttribute codeAttribute, Clazz clazz) {
    // create a node for each catch/finally statement
    // if more than one catch block target the same try block each subsequent node is linked to the
    // previous one with an edge that assumes the exception was not caught
    if (codeAttribute.exceptionTable != null) {
      ...
      for (int handlerOffset : handlersOffset) {
        ...
        // link the catch node to the starting node of the handler block with a true exception
        // assumption
      ...
      for (int currHandlerOffset : handlersOffset) {
        Set<ExceptionInfo> handledBlocks =
            Arrays.stream(codeAttribute.exceptionTable)
        ...
        for (ExceptionInfo currBlock : handledBlocks) {
          ...
          Optional<ExceptionInfo> followingCatch =
              Arrays.stream(codeAttribute.exceptionTable)
                  .skip(pos + 1)
                  .filter(e -> e.u2startPC <= currBlock.u2startPC && e.u2endPC >= currBlock.u2endPC)
                  .findFirst();

          if (followingCatch.isPresent()) {
            // if there is a following catch block we link it with a false assume exception edge
            // with the previous catch node type
            // i.e. the following catch block is considered only if the exception is not caught by
            // the block before
            JvmCatchCfaNode currNode = cfa.getFunctionCatchNode(signature, currHandlerOffset);
            JvmCatchCfaNode nextNode =
                cfa.getFunctionCatchNode(signature, followingCatch.get().u2handlerPC);
    ...
    // if the last node of a catch chain (i.e. linked only to the beginning of the corresponding
    // catch block and not to other catch nodes)is not a `finally` (catch type == 0) which always
    // throws
    // an exception in the end, add an edge to the exception exit block, since this is an
    // intra-procedural implementation
  ...
  /**
   * This {@link InstructionVisitor} creates nodes and edges of a {@link JvmCfa} for every
   * instruction it visits. For every instruction the node for its offset is added (if it did not
   * exist before) plus eventual additional nodes (i.e. in case of a branch instruction a node for
   * the code location of the jump is added).
   *
  ...
  private static class JvmIntraproceduralCfaFillerVisitor implements InstructionVisitor {
    ...
    public void visitConstantInstruction(
      ...
      // TODO: maybe method calls should have their unique edge type even for intra-procedural
      // implementation?
      connectStatement(offset, clazz, codeAttribute);
    }
    ...
    public void visitAnySwitchInstruction(
        ...
        CodeAttribute codeAttribute,
        int offset,
        SwitchInstruction tableSwitchInstruction) {
      try {
    ...
    /**
     * Performs the default operations needed for each instruction, must be called by all the other
     * specific addNode methods:
     *
     * <p>- Creates a new node for the current location if it does not exist.
     *
     * <p>- Links the previous edge if the previous instruction indicated that the control flows
     * through the two instructions.
     */
    private JvmCfaNode connect(int offset, Clazz clazz) {
      JvmCfaNode currentNode = cfa.addNodeIfAbsent(signature, offset, clazz);

      if (previousEdge != null) {
        // connect the previous edge if it exist (i.e. not first node and previous instruction is
        // the antecedent in the control flow)
    ...
    /**
     * A normal instruction, the control flows to the next one. This also includes function calls
     * since it's an intra-procedural implementation.
     */
    private void connectStatement(int offset, Clazz clazz, CodeAttribute methodCode) {
      JvmCfaNode currentNode = connect(offset, clazz);

      // create the new outgoing edge that will be connected to next node when the next instruction
      // is visited
    ...
    /**
     * we are not able to reconstruct the correct flow of the ret operation without knowing the
     * value of the variable on the stack, the best we can do is adding an edge to an unknown state
     * that might be modified when the information is available.
     */
    ...
    private void connectGoto(
        int offset, Clazz clazz, CodeAttribute methodCode, BranchInstruction instruction) {
      JvmCfaNode currentNode = connect(offset, clazz);
    ...
    /**
     * Creates two {@link JvmAssumeCfaEdge}: a true one to the branch target and a false one to the
     * next instruction.
     */
    private void connectBranch(
        int offset, Clazz clazz, CodeAttribute methodCode, BranchInstruction instruction) {
      JvmCfaNode currentNode = connect(offset, clazz);

      // create the branch not taken edge that will be connected to next node when the next
    ...
    /**
     * Creates a {@link JvmAssumeCaseCfaEdge} for each case of a switch statement and a {@link
     * JvmAssumeDefaultCfaEdge} to the default location node.
     */
    private void connectSwitch(
        int offset, Clazz clazz, CodeAttribute methodCode, SwitchInstruction instruction)
      ...
      // we already link the following node since its location belongs to a case/default (there is
      // always a default in the table)
      previousEdge = null;
      ...
      for (int index = 0; index < instruction.jumpOffsets.length; index++) {
        ...
        int caseAssumption =
            instruction instanceof TableSwitchInstruction
                ? ((TableSwitchInstruction) instruction).lowCase + index
                : ((LookUpSwitchInstruction) instruction).cases[index];
    ...
    /**
     * Add a node to the catch node at the handler location of the first {@link ExceptionInfo} in
     * the {@link CodeAttribute} exception table that is applicable to the current instruction's
     * offset.
     */
    private void connectThrow(int offset, Clazz clazz, CodeAttribute methodCode) {
      ...
      if (methodCode.exceptionTable != null) {
        ...
        // i.e. the first catch handler that is applicable to current offset since the jvm just
        // looks up the exception table in order
        // traversing the assume exception edges you can reach the correct catch block once the
        // exception type is known
        firstCatch =
            Arrays.stream(methodCode.exceptionTable)
      ...
      if (firstCatch.isPresent()) {
        new JvmInstructionCfaEdge(
            currentNode,
            cfa.getFunctionCatchNode(signature, firstCatch.get().u2handlerPC),
            methodCode,
            offset);
      }
      // since this is an intra-procedural implementation if the thrown exception is not caught just
      // add an edge to the exception exit node
      else {
        JvmCfaNode exitNode = cfa.getFunctionExceptionExitNode(signature, clazz);
