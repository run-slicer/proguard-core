...
public class JvmShallowHeapAbstractState<ReferenceT, StateT extends LatticeAbstractState<StateT>>
   ...
   */
  public JvmShallowHeapAbstractState(
      MapAbstractState<ReferenceT, StateT> referenceToObject,
  ...
  public void reduce(Set<Object> referencesToKeep) {
    ...
    logger.trace(
        "Reduced heap: retain {}, original heap size {}; new heap size {}; anyRemoved = {}.",
        referencesToKeep.size(),
        originalSize,
        referenceToObject.size(),
        anyRemoved);
  }
  ...
  public void expand(JvmHeapAbstractState<StateT> otherState) {
    ...
    ((JvmShallowHeapAbstractState<ReferenceT, StateT>) otherState)
        .referenceToObject.forEach(referenceToObject::putIfAbsent);
  }
  ...
  public JvmShallowHeapAbstractState<ReferenceT, StateT> join(
      JvmHeapAbstractState<StateT> abstractState) {
    JvmShallowHeapAbstractState<ReferenceT, StateT> other =
        (JvmShallowHeapAbstractState<ReferenceT, StateT>) abstractState;
    MapAbstractState<ReferenceT, StateT> newReferenceToState =
  ...
  public boolean isLessOrEqual(JvmHeapAbstractState<StateT> abstractState) {
    return abstractState instanceof JvmShallowHeapAbstractState
        && referenceToObject.isLessOrEqual(
            ((JvmShallowHeapAbstractState<ReferenceT, StateT>) abstractState).referenceToObject);
  }
  ...
  public JvmShallowHeapAbstractState<ReferenceT, StateT> copy() {
    return new JvmShallowHeapAbstractState<>(
        referenceToObject.copy(), referenceClass, defaultValue);
  }
}
